...
void Manual_task(void *p_arg)
{
	...
	while(1)
	{
		...
		{
			if((QianJin==0)||(rece3_buf[0]==0x02))//前进
			{
				CanSend_data(1);
				if(QianJin==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x02) PWM = (rece3_buf[1]+pwm_add)*3;
				Motor_Zzhuan(1,PWM);
				Motor_Zzhuan(2,PWM);
				Motor_Zzhuan(3,PWM);
				Motor_Zzhuan(4,PWM);						
			}
			else if((HouTui==0)||(rece3_buf[0]==0x01))//后退
			{
				CanSend_data(2);
				if(HouTui==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x01) PWM = (rece3_buf[1]+pwm_add)*3;
				Motor_Fzhuan(1,PWM);
				Motor_Fzhuan(2,PWM);
				Motor_Fzhuan(3,PWM);
				Motor_Fzhuan(4,PWM);			
			}
			else if((ZuoYi==0)||(rece3_buf[0]==0x03))//左移
			{
				CanSend_data(3);
				if(ZuoYi==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x03) PWM = (rece3_buf[1]+pwm_add)*3;
				Motor_Fzhuan(1,PWM);
				Motor_Zzhuan(2,PWM);
				Motor_Zzhuan(3,PWM);
				Motor_Fzhuan(4,PWM);
			}
			else if((YouYI==0)||(rece3_buf[0]==0x04))//右移
			{
				CanSend_data(4);
				if(YouYI==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x04) PWM = (rece3_buf[1]+pwm_add)*3;
				Motor_Zzhuan(1,PWM);
				Motor_Fzhuan(2,PWM);
				Motor_Fzhuan(3,PWM);
				Motor_Zzhuan(4,PWM);
			}
			else if((ZuoShang==0)||(rece3_buf[0]==0x07))//左上
			{
				CanSend_data(5);
				if(ZuoShang==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x07) PWM = (rece3_buf[1]+pwm_add)*3;							
				Motor_Zzhuan(1,PWM);
				MotoStop(2);
				MotoStop(3);
				Motor_Zzhuan(4,PWM);
			}
			else if((ZuoXia==0)||(rece3_buf[0]==0x05))//左下
			{
				CanSend_data(6);
				if(ZuoXia==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x05) PWM = (rece3_buf[1]+pwm_add)*3;			
				Motor_Fzhuan(1,PWM);
				MotoStop(2);
				MotoStop(3);
				Motor_Fzhuan(4,PWM);			
			}
			else if((YouShang==0)||(rece3_buf[0]==0x08))//右上
			{
				CanSend_data(7);
				if(YouShang==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x08) PWM = (rece3_buf[1]+pwm_add)*3;			
				MotoStop(1);			
				Motor_Zzhuan(2,PWM);
				Motor_Zzhuan(3,PWM);
				MotoStop(4);			
			}
			else if((YouXia==0)||(rece3_buf[0]==0x06))//右下
			{
				CanSend_data(8);
				if(YouXia==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x06) PWM = (rece3_buf[1]+pwm_add)*3;			
				MotoStop(1);
				Motor_Fzhuan(2,PWM);
				Motor_Fzhuan(3,PWM);
				MotoStop(4);	
			}
			else if((ZuoXuan==0)||(rece3_buf[0]==0x09))//左旋
			{
				CanSend_data(9);
				if(ZuoXuan==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x09) PWM = (rece3_buf[1]+pwm_add)*3;			
				Motor_Fzhuan(1,PWM);
				Motor_Zzhuan(2,PWM);
				Motor_Fzhuan(3,PWM);
				Motor_Zzhuan(4,PWM);			
			}
			else if((YouXuan==0)||(rece3_buf[0]==0x0a))//右旋
			{
				CanSend_data(10);
				if(YouXuan==0)	PWM = SouDongSuDu;
				if(rece3_buf[0]==0x0a) PWM = (rece3_buf[1]+pwm_add)*3;			
				Motor_Zzhuan(1,PWM);
				Motor_Fzhuan(2,PWM);
				Motor_Zzhuan(3,PWM);
				Motor_Fzhuan(4,PWM);
...
void float_task(void *p_arg)
{
	u8 num=0;
	u8 yuyin_flag=0;
	u16 cidaohang_now = 0,cidaohang_last = 0,num_dianliang=0;
	OS_ERR err;
	p_arg = p_arg;
	delay_ms(20);
	yuyin_flag=YinLiang;
	while(1)
	{
		...
		if(MAh<3271)//满容量8179，小于满容量的40%报警
		{
			num_dianliang++;
			if(num_dianliang==1500)
			{
				num_dianliang = 0;
				speek("电池电量不足，请及时充电！");	
			}			
	...
	while(1)
	{
		...
		if(flag_szd==0)
		{
			...
			{				
				if(dir==0)//正方向
				{
					if(((QianZuoLeiDa<=ld_juli)||(QianYouLeiDa<=ld_juli)||(QianZhongLeiDa<=ld_juli))&&(Jhwai_flag == 0))
					{
						HmiTaskState = 2;
						Jhwai_flag = 1;
						MotoStop(5);
					...
					else if(((QianZuoLeiDa>ld_juli)&&(QianYouLeiDa>ld_juli)&&(QianZhongLeiDa>ld_juli))&&(Jhwai_flag == 1))
					{
				...
				{
					if(((HouZuoLeiDa<=ld_juli)||(HouYouLeiDa<=ld_juli)||(HouZhongLeiDa<=ld_juli))&&(Jhwai_flag == 0))
					{
						HmiTaskState = 2;
						Jhwai_flag = 1;
						MotoStop(5);
					...
					else if(((HouZuoLeiDa>ld_juli)&&(HouYouLeiDa>ld_juli)&&(HouZhongLeiDa>ld_juli))&&(Jhwai_flag == 1))
					{
						OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);
						HmiTaskState = 5;
						Jhwai_flag = 0;
				...
				{
					if(((ZuoQianLeiDa<=ld_juli)||(ZuoHouLeiDa<=ld_juli)||(Zuo2LeiDa<=ld_juli)||(Zuo3LeiDa<=ld_juli))&&(Jhwai_flag == 0))
					{
						HmiTaskState = 2;
						Jhwai_flag = 1;
						MotoStop(5);
					...
					else if(((ZuoQianLeiDa>ld_juli)&&(ZuoHouLeiDa>ld_juli)&&(Zuo2LeiDa>ld_juli)&&(Zuo3LeiDa>ld_juli))&&(Jhwai_flag == 1))
					{
						OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);
						HmiTaskState = 5;
						Jhwai_flag = 0;
				...
				{
					if(((YouQianLeiDa<=ld_juli)||(YouHouLeiDa<=ld_juli)||(You2LeiDa<=ld_juli)||(You3LeiDa<=ld_juli))&&(Jhwai_flag == 0))
					{
						HmiTaskState = 2;
						Jhwai_flag = 1;
						MotoStop(5);
					...
					else if(((YouQianLeiDa>ld_juli)&&(YouHouLeiDa>ld_juli)&&(You2LeiDa>ld_juli)&&(You3LeiDa>ld_juli))&&(Jhwai_flag == 1))
					{
						OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);
						HmiTaskState = 5;
						Jhwai_flag = 0;
				...
				{
					if(((YouQianLeiDa<=ld_juli)||(YouHouLeiDa<=ld_juli)||(ZuoQianLeiDa<=ld_juli)||(ZuoHouLeiDa<=ld_juli)
						||(Zuo2LeiDa<=ld_juli)||(Zuo3LeiDa<=ld_juli)||(You2LeiDa<=ld_juli)||(You3LeiDa<=ld_juli))&&(Jhwai_flag == 0))
					{
						HmiTaskState = 2;
						Jhwai_flag = 1;
						MotoStop(5);
					...
					else if(((YouQianLeiDa>ld_juli)&&(YouHouLeiDa>ld_juli)&&(ZuoQianLeiDa>ld_juli)&&(ZuoHouLeiDa>ld_juli)
						&&(Zuo2LeiDa>ld_juli)&&(Zuo3LeiDa>ld_juli)&&(You2LeiDa>ld_juli)&&(You3LeiDa>ld_juli))&&(Jhwai_flag == 1))
					{
						OSTimeDlyHMSM(0,0,1,0,OS_OPT_TIME_HMSM_STRICT,&err);
						HmiTaskState = 5;
						Jhwai_flag = 0;
...
{	
	while(1)
	{
		...
		if((flag_szd==1)&&(SouDongBiZhang))
		{
			if(QianJin==0)//前进
			{
				if(((QianZuoLeiDa<Manual_juli)||(QianYouLeiDa<Manual_juli)||(QianZhongLeiDa<Manual_juli)||(QianFangZhuang==1)))
				{
					speek_flag=1;
					MotoStop(5);
				}
			...
			if(HouTui==0)//后退
			{
				if(((HouZuoLeiDa<Manual_juli)||(HouYouLeiDa<Manual_juli)||(HouZhongLeiDa<Manual_juli)||(HouFangZhuang==1)))
				{
					speek_flag=1;
					MotoStop(5);					
				}	
			...
			if(ZuoYi==0)//左移
			{
				if(((ZuoQianLeiDa<Manual_juli)||(ZuoHouLeiDa<Manual_juli)||(Zuo2LeiDa<Manual_juli)||(Zuo3LeiDa<Manual_juli)))
				{
					speek_flag=1;
					MotoStop(5);				
				}	
			...
			if(YouYI==0)//右移
			{
				if(((YouQianLeiDa<Manual_juli)||(YouHouLeiDa<Manual_juli)||(You2LeiDa<Manual_juli)||(You3LeiDa<Manual_juli)))
				{
					speek_flag=1;
					MotoStop(5);					
				}	
			...
			if(ZuoXuan==0)//左旋
			{
				if((ZuoQianLeiDa<Manual_juli)||(ZuoHouLeiDa<Manual_juli)||(You2LeiDa<Manual_juli)||(You3LeiDa<Manual_juli)
					||(YouQianLeiDa<Manual_juli)||(YouHouLeiDa<Manual_juli)||(Zuo2LeiDa<Manual_juli)||(Zuo3LeiDa<Manual_juli))
				{
					speek_flag=1;
					MotoStop(5);					
				}
			...
			if(ZuoShang==0)//左上
			{
				if(((QianZuoLeiDa<Manual_juli)||(QianYouLeiDa<Manual_juli)||(QianZhongLeiDa<Manual_juli)||(ZuoQianLeiDa<Manual_juli)||(ZuoHouLeiDa<Manual_juli)
					||(Zuo2LeiDa<Manual_juli)||(Zuo3LeiDa<Manual_juli)))
				{
					speek_flag=1;
					MotoStop(5);					
				}
			...
			if(YouShang==0)//右上
			{
				if(((QianZuoLeiDa<Manual_juli)||(QianYouLeiDa<Manual_juli)||(QianZhongLeiDa<Manual_juli)||(YouQianLeiDa<Manual_juli)||(YouHouLeiDa<Manual_juli)
					||(You2LeiDa<Manual_juli)||(You3LeiDa<Manual_juli)))
				{
					speek_flag=1;
					MotoStop(5);					
				}
...
void DEMO1_task(void *p_arg)
{
	while(1)
	{
		if(startAsk && (jiting_flag == 0) &&(flag_tz == 0))
		{
			if(HmiTask == 0)
			{
				JieMianHao = 21;
