...
u8  startAsk=0,stopAsk=0,zyAsk=0,yyAsk=0,flag_qd=0,flag_tz=0;
u8 	car_statu=0;
u8 CAN1_Sebuf[5]={1,0,1,0,0x55};
u16 xunzhuan_speed = 150,pingyi_speed = 150;
u8 jiting_flag = 0,zuoyi_flag = 0,youyi_flag = 0,zuoxuan_flag = 0,youxuan_flag = 0;
u8 quxiaorenwu_flag = 0,flag_szd=1;
void QJ(void)
{	
	CanSend_data(1);
}
...
{	
	CanSend_data(3);
	Motor_SetVul(1,pingyi_speed);
	Motor_SetVul(2,pingyi_speed);
	Motor_SetVul(3,pingyi_speed);
	Motor_SetVul(4,pingyi_speed);	
	speed = 100;
	while((cdh_16_qian!=0xffff)||(cdh_16_hou!=0xffff)||(flag_szd == 1))
	{	
		zuoyi_flag = 0;
		...
		if(quxiaorenwu_flag)	break;
		delay(0,0,0,10);
	}
	while(!(((cdh_16_qian<0xffff)&&(abs(front_cdh16.Num)>5))||((cdh_16_hou<0xffff)&&(abs(back_cdh16.Num)>5)))||(flag_szd == 1))
	{
		if((Jhwai_flag==0)&&(jiting_flag == 0)&&(flag_qd==1))
		{
			CanSend_data(3);
...
{		
	...
	Motor_SetVul(4,pingyi_speed);	
	speed = 100;
	while((cdh_16_qian!=0xffff)||(cdh_16_hou!=0xffff)||(flag_szd == 1))//先出轨
	{
		youyi_flag = 0;
	...
	while(!(((cdh_16_qian<0xffff)&&(abs(front_cdh16.Num)>5))||((cdh_16_hou<0xffff)&&(abs(back_cdh16.Num)>5)))||(flag_szd == 1))//再上轨
	{			
		if((Jhwai_flag==0)&&(jiting_flag == 0)&&(flag_qd==1))
		{
			CanSend_data(4);
...
{	
	u8 i = 0;
	CanSend_data(10);
	Motor_SetVul(1,xunzhuan_speed);
	Motor_SetVul(2,xunzhuan_speed);
	Motor_SetVul(3,xunzhuan_speed);
	Motor_SetVul(4,xunzhuan_speed);
	for(i=0;i<num;i++)
	{
		while((cdh_16_qian!=0xffff)||(cdh_16_hou!=0xffff)||(flag_szd == 1))
		{
		...
		while(!(((cdh_16_qian<0xffff)&&(abs(front_cdh16.Num)>3))||((cdh_16_hou<0xffff)&&(abs(back_cdh16.Num)>3)))||(flag_szd == 1))
		{
			if((Jhwai_flag==0)&&(jiting_flag == 0)&&(flag_qd==1))
			{
				CanSend_data(10);
...
void PID_Adjust(u16 j_speed,float kp,float ki,float kd)
{
	...
	if(dir == 0)
	{
		...
		else
		{
			...
			Motor_SetVul(1,speed1);
			Motor_SetVul(2,speed2);
			Motor_SetVul(3,speed3);
			Motor_SetVul(4,speed4);								
		}
	}
	else if(dir == 1)
	{
